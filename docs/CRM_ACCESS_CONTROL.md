...
Se ha implementado un sistema de control de acceso granular que permite a usuarios tipo `lawyer` y `agent` acceder al CRM, mientras que los usuarios tipo `admin` tienen acceso exclusivo al panel de administración.

## 🎯 Objetivo

- **Usuarios `lawyer` y `agent`**: Pueden acceder al CRM
- **Usuarios `admin`**: NO pueden acceder al CRM, solo al panel de administración
...
- Si el usuario es `admin` y no está en `allowedRoles`, se deniega el acceso explícitamente
- Mantiene compatibilidad con `requireAdmin` para rutas de administración

### 2. Rutas del CRM (`src/App.tsx`)

Todas las rutas del CRM ahora usan `allowedRoles={['lawyer', 'agent']}` en lugar de `requireAdmin`:
...
3. Al intentar acceder a `/crm/*`:
   ...
   - Si es `admin` → Acceso denegado con mensaje específico
...
3. Al intentar acceder a `/crm/*`:
   - `ProtectedRoute` verifica que el rol esté en `allowedRoles`
   - Como `admin` no está en `['lawyer', 'agent']` → Acceso denegado
...
### Acceso Denegado para Admin intentando acceder al CRM:
```
Acceso Denegado
Los administradores no tienen acceso a esta sección
```
...
3. **Bloqueo explícito de admin**: Los admins no pueden acceder al CRM aunque intenten navegar directamente
...
3. **Usuario `admin`**:
   ...
   - ✅ Debe ver el switch Admin/CRM
   - ✅ NO debe poder acceder a `/crm/*` (debe mostrar mensaje de acceso denegado)
